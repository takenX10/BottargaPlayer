...
public class Alphabeta {
    ...
    public MNKCell iterative_negamax(){
        ...
        for(currentMaxDepth = 1;;currentMaxDepth++){
            ...
            if(this.last_depth || this.endNegamax || this.bestCell.eval.status == EvalStatus.WIN || this.bestCell.eval.status == EvalStatus.LOSE){
                break;
            }
        }
    ...
    private void maxValue(){
        ...
        this.bestCell = new CustomMNKCell(max.cell, false, new CustomScore(max.eval.score, max.eval.status),false);
    }
}